...
export function buildDiegoData(transactions, stockItems = [], furnitureItems = [], stockProperties = [], estadias = [], limpiezas = [], kanbanTasks = []) {
  ...
  return {
    ...
    stock: stockItems.map(s => ({
      ...
      umbralPctEnUso: s.umbralPctEnUso ?? null,
    })),
    furniture: furnitureItems.map(f => ({
      id: f.id, property: f.property, zone: f.zone, category: f.category, name: f.name,
      qty: f.qty, umbralUnidades: f.umbralUnidades,
    })),
    stockProperties: stockProperties.map(p => ({
      id: p.id, name: p.name, zones: p.zones.map(z => ({ id: z.id, label: z.label })),
      color: p.color ?? null, kind: p.kind ?? null,
    })),
    estadias: estadias.map(e => ({
      id: e.id, property: e.property, guestName: e.guestName,
      checkIn: e.checkIn, checkOut: e.checkOut, guestCount: e.guestCount, monto: e.monto, notes: e.notes,
    })),
    limpiezas: limpiezas.map(l => ({
      id: l.id, property: l.property, date: l.date, notes: l.notes, done: l.done,
    })),
